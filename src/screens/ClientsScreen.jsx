  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        {client.website_url && <span style={{ fontSize: 11, color: theme.blue }}>🌐 Sitio web</span>}
      </div>
      <div style={{ fontSize: 10, color: theme.dim }}>
        Cliente desde {new Date(client.started_at).toLocaleDateString("es-MX", { year: "numeric", month: "short", day: "numeric" })}
      </div>
    </div>
  );
}
...
function NewClientModal({ workspaceId, onCreated, onClose }) {
  const [form, setForm] = useState({
    name: "", contact_name: "", whatsapp: "", email: "", city: "", notes: ""
  });
  ...
  const inputStyle = {
    ...
    background: theme.s1, border: `1px solid ${theme.border}`, borderRadius: 8,
    color: theme.text, fontSize: 13, padding: "9px 12px", outline: "none"
  };

  const labelStyle = { fontSize: 11, color: theme.muted, display: "block", marginBottom: 4 };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: theme.s2, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 28, width: "min(480px,100%)", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: theme.text, marginBottom: 20 }}>Nuevo cliente</div>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={labelStyle}>Nombre del negocio *</label>
            <input style={inputStyle} value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Ej: Restaurante El Rincón" autoFocus required />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={labelStyle}>Contacto principal</label>
              <input style={inputStyle} value={form.contact_name} onChange={(e) => set("contact_name", e.target.value)} placeholder="Nombre" />
            </div>
            <div>
              <label style={labelStyle}>Ciudad</label>
              <input style={inputStyle} value={form.city} onChange={(e) => set("city", e.target.value)} placeholder="Reynosa" />
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={labelStyle}>WhatsApp</label>
              <input style={inputStyle} value={form.whatsapp} onChange={(e) => set("whatsapp", e.target.value)} placeholder="+52 899..." />
            </div>
            <div>
              <label style={labelStyle}>Correo</label>
              <input style={inputStyle} value={form.email} onChange={(e) => set("email", e.target.value)} type="email" placeholder="negocio@gmail.com" />
            </div>
          </div>
          <div>
            <label style={labelStyle}>Notas</label>
            <textarea style={{ ...inputStyle, resize: "vertical", minHeight: 72 }} value={form.notes} onChange={(e) => set("notes", e.target.value)} placeholder="Contexto general del cliente..." />
          </div>
